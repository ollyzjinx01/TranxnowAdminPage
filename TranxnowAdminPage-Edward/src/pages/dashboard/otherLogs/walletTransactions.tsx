...
import { Td, Tr, Badge, Flex, Avatar } from "@chakra-ui/react";
...
const Status: FC<Tstatus> = ({ color, status }) => {
  return (
    <Badge colorScheme={color} py={".3em"} px={"1em"} borderRadius={"1em"}>
      {status}
    </Badge>
  );
};
...
const WalletTransactions = () => {
  const { data } = useFetch({
    endpoint: "admin/Log/wallettransaction/get",
    body: {
      _populate: "agent",
    },
  });
  ...
  const filteredData = filterData(["email", "phoneNumber", "status", "amount"]);
  ...
  return (
    <DashboardLayout
      headerTop={"Wallet transaction Logs"}
      tableId="walletTransactionTable"
    >
      {filteredData?.length === 0 ? (
        <NoData />
      ) : (
        <DefaultTable tableHeader={header} tableId="walletTransactionTable">
          {filteredData?.map(
            (
              {
                agent,
              ...
              if (status === "00") {
                ...
                itemStatus = <Status color="red" status={"Failed"} />;
              } else if (status === "02") {
                  ...
                  <Status
                    ...
                    status={
                      <Flex
                        ...
                        cursor={"pointer"}
                      >
                        Reprocess <MdRotateLeft />
                      </Flex>
                    }
                  />
                );
              } else if (status === "10") {
                itemStatus = <Status color="red" status={"Fraud"} />;
              } else if (status === "09") {
                itemStatus = <Status color="teal" status={"Closed"} />;
              }
