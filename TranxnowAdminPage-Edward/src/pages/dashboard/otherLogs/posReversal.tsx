...
const PosReversal = () => {
  const { data } = useFetch({
    endpoint: "admin/Log/posreverse/get",
    body: {
      _populate: "agent",
    },
  });
  ...
  return (
    <DashboardLayout headerTop={"POS Reversal"} tableId="POSReversalTable">
      {itemData?.length === 0 ? (
        <NoData />
      ) : (
        <DefaultTable tableHeader={header} tableId="POSReversalTable">
          {itemData?.map(
            (
              {
                agent,
