...
const DatasData = () => {
  ...
  const filteredData = filterData(["phoneNumber", "ammount", "network"]);
  ...
  return (
    ...
    >
      {filteredData?.length === 0 ? (
        ...
        <DefaultTable tableHeader={header} tableId="dataTable">
          {filteredData?.map(
            (
              { user, phoneNumber, PlanName, status, amount, email, createdAt },
              index
            ) => {
              let network;
